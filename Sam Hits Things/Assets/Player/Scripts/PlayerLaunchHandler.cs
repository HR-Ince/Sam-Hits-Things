...
public class PlayerLaunchHandler : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if(anim == null) { Debug.LogError("Animator missing from " + gameObject.name); }
    ...
    private void Update()
    {
        if (!allowLaunchInAir && grounding.IsGrounded || allowLaunchInAir)
        {
            if (input.Pressed)
            {
                if (allowLaunchInAir)
        ...
        if (grounding.IsGrounded && anim.GetBool("isFalling"))
        {
